...
class VacationAPIView(APIView):
    ...
    def get(self, request, pk=None, format=None):
        if pk:
            obj = get_object_or_404(Vacation, pk=pk, deleted_at__isnull=True)
            serializer = VacationSerializer(obj)
            return Response(serializer.data, status=status.HTTP_200_OK)
        
        objs = Vacation.objects.filter(deleted_at__isnull=True)
        serializer = VacationSerializer(objs, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def put(self, request, pk, format=None):
        obj = get_object_or_404(Vacation, pk=pk, deleted_at__isnull=True)
        serializer = VacationSerializer(obj, data=request.data)
        if serializer.is_valid():
    ...
    def patch(self, request, pk, format=None):
        obj = get_object_or_404(Vacation, pk=pk, deleted_at__isnull=True)
        serializer = VacationSerializer(obj, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save(updated_at=timezone.now())
            return Response(serializer.data, status=status.HTTP_200_OK)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _delete_multiple(self, request):
        try:
            ...
            existing_objs = Vacation.objects.filter(
                id__in=ids,
            ...
            return Response({
                'message': f'{updated} vacations deleted successfully',
            ...
            traceback.print_exc()
            return Response(
                {'error': f'Error deleting vacations: {str(e)}'},
