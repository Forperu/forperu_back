...
class HolidayAPIView(APIView):
    ...
    def get(self, request, pk=None, format=None):
        if pk:
            obj = get_object_or_404(Holiday, pk=pk, deleted_at__isnull=True)
            serializer = HolidaySerializer(obj)
            return Response(serializer.data, status=status.HTTP_200_OK)
        
        objs = Holiday.objects.filter(deleted_at__isnull=True)
        serializer = HolidaySerializer(objs, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def put(self, request, pk, format=None):
        obj = get_object_or_404(Holiday, pk=pk, deleted_at__isnull=True)
        serializer = HolidaySerializer(obj, data=request.data)
        if serializer.is_valid():
    ...
    def _delete_multiple(self, request):
        try:
            ...
            existing_objs = Holiday.objects.filter(
                id__in=ids,
            ...
            return Response({
                'message': f'{updated} holidays deleted successfully',
            ...
            traceback.print_exc()
            return Response(
                {'error': f'Error deleting holidays: {str(e)}'},
