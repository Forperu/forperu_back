...
class AttendanceTypeAPIView(APIView):
  ...
  def get(self, request, pk=None, format=None):
    if pk:
      obj = get_object_or_404(AttendanceType, pk=pk, deleted_at__isnull=True)
      serializer = AttendanceTypeSerializer(obj)
      return Response(serializer.data, status=status.HTTP_200_OK)
        
    objs = AttendanceType.objects.filter(deleted_at__isnull=True)
    serializer = AttendanceTypeSerializer(objs, many=True)
    return Response(serializer.data, status=status.HTTP_200_OK)
  ...
  def patch(self, request, pk, format=None):
    obj = get_object_or_404(AttendanceType, pk=pk, deleted_at__isnull=True)
    serializer = AttendanceTypeSerializer(obj, data=request.data, partial=True)
    if serializer.is_valid():
      serializer.save(updated_at=timezone.now())
      return Response(serializer.data, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
  ...
  def _delete_multiple(self, request):
    try:
      ...
      existing_objs = AttendanceType.objects.filter(
        id__in=ids,
      ...
      return Response({
        'message': f'{updated} attendance types deleted successfully',
